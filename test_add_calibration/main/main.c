...
static void rx_mhz14a_task(void *arg)
{
    ...
    while (1) {
        ...
        if (rxBytes > 0) {
            ...
            float calibrated_co2 = calibrate_co2(co2_concentration);  // Hiệu chuẩn giá trị CO2
            ESP_LOGI(RX_TASK_TAG, "\n\n\tCalibrated CO2 Concentration: %.2f\n\n", calibrated_co2);
        }
    }
    free(data);
}
...
static void rx_pms7003_task(void *arg)
{
    ...
    while (1) {
        ...
        if (err == ESP_OK) {
            // Hiệu chuẩn các giá trị PM
            float calibrated_pm1_0 = calibrate_pm1_0(pm1_0);
            float calibrated_pm2_5 = calibrate_pm2_5(pm2_5);
            float calibrated_pm10 = calibrate_pm10(pm10);

            ESP_LOGI(TAG_PMS7003, "Calibrated PM1.0: %.2f ug/m3, Calibrated PM2.5: %.2f ug/m3, Calibrated PM10: %.2f ug/m3",
                     calibrated_pm1_0, calibrated_pm2_5, calibrated_pm10);
        } else {
...
// Calibration functions for PMS7003
float calibrate_pm1_0(float raw_pm1_0) {
    return raw_pm1_0 + 5.0;
}

float calibrate_pm2_5(float raw_pm2_5) {
    return raw_pm2_5 + 3.0;
}
